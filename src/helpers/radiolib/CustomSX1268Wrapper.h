...
public:
  ...
  // Hardware duty cycling support for SX1268 - now with real RadioLib API  
  ...
  bool startReceiveDutyCycle(uint32_t rxPeriod, uint32_t sleepPeriod) override {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      Serial.printf("DEBUG: SX1268 hardware duty cycle started: RX=%dms, Sleep=%dms\n", 
                    rxPeriod, sleepPeriod);
      return true;
    } else {
      Serial.printf("ERROR: SX1268 startReceiveDutyCycle failed: %d\n", state);
      return false;
    }
  }
  ...
  bool startChannelActivityDetection() override {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      Serial.println("DEBUG: SX1268 CAD started");
      return true;
    } else {
      Serial.printf("ERROR: SX1268 CAD start failed: %d\n", state);
      return false;
    }
  }
  ...
  bool isChannelActivityDetected() override {
    ...
    if (cadDetected) {
      Serial.println("DEBUG: SX1268 CAD activity detected");
    }
  ...
  bool startPreambleDetection() override {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      Serial.println("DEBUG: SX1268 preamble detection started");
      return true;
    } else {
      Serial.printf("ERROR: SX1268 preamble detection start failed: %d\n", state);
      return false;
    }
  }
  ...
  bool isPreambleDetected() override {
    ...
    if (preambleDetected) {
      Serial.println("DEBUG: SX1268 preamble detected");
    }
