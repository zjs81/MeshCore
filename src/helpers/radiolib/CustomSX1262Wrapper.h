...
public:
  ...
  // Hardware duty cycling support for SX1262 - now with real RadioLib API
  ...
  bool startReceiveDutyCycle(uint32_t rxPeriod, uint32_t sleepPeriod) override {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      Serial.printf("DEBUG: SX1262 hardware duty cycle started: RX=%dms, Sleep=%dms\n", 
                    rxPeriod, sleepPeriod);
      return true;
    } else {
      Serial.printf("ERROR: SX1262 startReceiveDutyCycle failed: %d\n", state);
      return false;
    }
  }
  ...
  bool startChannelActivityDetection() override {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      Serial.println("DEBUG: SX1262 CAD started");
      return true;
    } else {
      Serial.printf("ERROR: SX1262 CAD start failed: %d\n", state);
      return false;
    }
  }
  ...
  bool isChannelActivityDetected() override {
    ...
    if (cadDetected) {
      Serial.println("DEBUG: SX1262 CAD activity detected");
    }
  ...
  bool startPreambleDetection() override {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      Serial.println("DEBUG: SX1262 preamble detection started");
      return true;
    } else {
      Serial.printf("ERROR: SX1262 preamble detection start failed: %d\n", state);
      return false;
    }
  }
  ...
  bool isPreambleDetected() override {
    ...
    if (preambleDetected) {
      Serial.println("DEBUG: SX1262 preamble detected");
    }
